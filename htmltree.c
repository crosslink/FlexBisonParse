...
void outputHTMLHelper (Node node);

/* This function is mutually recursive with outputHTMLHelper() */
void outputNode (Node node)
{
    ...
    rname =
        ...
        /* For the following, the tag is already output by outputHTMLHelper: */
        node->type == Heading       ? 0 :
        node->type == List          ? 0 :
        node->type == LinkEtc       ? 0 :
...
char* outputHTML (Node node, int initialBufferSize)
{
    fb_create_new_buffer (initialBufferSize);
    outputHTMLHelper (node);
